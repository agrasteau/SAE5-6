...
export const AuraPlay = forwardRef(
  ({ canvasRef, activityId, meta = {} }: any, ref: any) => {
    ...
    switch (activity.type) {
      ...
      case ActivityType.augmentationFromPhysical: {
        ARViewComponent = (
          <AugmentationFromPhysicalARView
          activityId={activityId}
          activity={activity}
          meta={meta}
          canvasRef={canvasRef}
...
function AugmentationFromPhysicalARView({
  activityId,
  activity,
  meta,
  canvasRef,
  ...
  arRef: any;
  meta: Record<string, any>;
  displayBlackScreen : boolean;
}) {
  const log = useLogger('AugmentationFromPhysicalARView');

  const [url, setUrl] = useState<string | undefined>(undefined);

  const [mediaIndex, setMediaIndex] = useState(-1); // Initially, no media is displayed
  ...
  const [showAuras, setShowAuras] = useState(true); // Manage aura visibility with state
  const displayMedias = () => {
    setMediaIndex((prevIndex) => {
      const nextIndex = prevIndex + 1; // Move to the next media
      log.debug('allAuras:', allAuras);
      log.debug('allAuras.length:', allAuras.length);
      log.debug('nextIndex:', nextIndex);
      
      if (nextIndex < allAuras.length) {
        return nextIndex; // Show the next media
      } else {
        const zero = 0;
        log.debug('Resetting to first media');
        return zero; // Reset to the first media after the last one
      }
    });
  };
  ...
  //log.debug('Render', activity, meta, url);
    ...
    const toggleFullscreen = useCallback(() => {
      ...
      logger.debug('test element', element);
      if (!element) {
        return;
      }
      if (!isFullscreen) {
    ...
    <>
      <Html>
        <Space direction="horizontal">
          {/* <Button
            ...
            type="primary"
            icon={<CheckOutlined />}
            onClick={displayMedias}  
            style={{
              position: 'fixed', // Pin the button relative to the viewport
              bottom: '45vh',     // 2% of the viewport height from the bottom
              left: '45vw',       // 2% of the viewport width from the left
      ...
      <ARView
        ...
        <ARAnchor
          ...
          {( 
            <AuraListAugmentationFromphysical
              canvasRef={canvasRef}
              activity={activity}
              activityId={activityId}
              anchoring={anchoring}
              ...
              mode={AuraMode.ARVIEW}
              showAuras={showAuras}
              mediaIndex={mediaIndex} // Pass index to AuraList
            />
          )}
        </ARAnchor>
      </ARView>
...
function AuraListAugmentationFromphysical({
  canvasRef,
  activity,
  activityId,
  anchoring,
  ...
  // Display only the current media based on mediaIndex
  const currentAura = auras[mediaIndex] ? [auras[mediaIndex]] : [];

  return (
    activity && (
      <group matrixAutoUpdate={false}>
        {currentAura.map((aura) => (
          <Aura
            key={aura.id}
            id={aura.id}
            type={aura.type}
  ...
  function AuraList({
    ...
    const visibleAuras = auras.slice(0, mediaIndex); // Show only up to mediaIndex
