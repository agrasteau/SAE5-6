...
export default function ActivityMenu() {
  ...
  useEffect(() => {
    if (addDefaults?.id && markerCfg?.marker) {
      ...
      // Vérification que 'auras' est bien un tableau avant d'utiliser forEach
      if (Array.isArray(auras)) {
        ...
        console.error("AurasDefaults is not an array or undefined for type:", addDefaults.type);
      }
    }
      ...
      <Card
        ...
        onClick={() => {
          if (!isDisabled) {
            ...
            if (
              type === ActivityType.Validation ||
              type === ActivityType.Association || type === ActivityType.augmentationFromPhysical
            ) {
              setAddDefaults({ id, type });
            }
